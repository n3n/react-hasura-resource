import { selectQuery, countQuery, bulkQuery } from './queries';
...
export async function getResource<TResult = unknown>(
  _key: string,
  params: GetResourceFnProps
...
export async function getResourceList<TResult = unknown[]>(
  _key: string,
  params: GetResourceListFnProps
): Promise<TResult | null> {
  let finalQuery: any = {};
  ...
  const { schema, tableName } = parse(_tableName);
  const { pagination = { perPage: 20, page: 1 }, sort = { field: [primaryKey], order: 'asc' } } = args;
  ...
  finalSelectQuery.args.order_by = {
    column: sort.field,
    type: typeof sort.order === 'undefined' ? 'asc' : sort.order.toLowerCase(),
  };
